              ...
              <tbody>
                @foreach($report->files as $file)
                 <tr align='center'>
                  ...
                  @endif
                  <td>
                   <form method="post" action="{{route('file.destroy', [$file->id])}}" style="margin-top:3%">
                        @method('DELETE')
                        @csrf
                        <button type="submit" class="btn btn-danger btn-sm" onclick="this.form.submit()" >  حذف</button>
                    </form>
                  </td>
                </tr>
              @endforeach
